...
def tools_health_check_job():
    """Job que verifica saúde das tools e atualiza ranking"""
    try:
        from app.agents.tools_health import ToolsHealthAgent
        from app.agents.base import TaskInput
        import uuid
        from datetime import datetime
        print(f"[ToolsHealthScheduler] Verificando tools em {datetime.utcnow().isoformat()}")
        agent = ToolsHealthAgent()
        task = TaskInput(
            task_id=str(uuid.uuid4()),
            mission_id=str(uuid.uuid4()),
            agent_id="tools_health",
            objective="verificar saúde de todas as tools - job agendado"
        )
        output = agent.execute(task)
        print(f"[ToolsHealthScheduler] Resultado: {output.summary}")
    except Exception as e:
        print(f"[ToolsHealthScheduler] Erro: {e}")

def start_tools_health_scheduler():
    """Inicia scheduler que verifica tools a cada 10 minutos"""
    try:
        scheduler = get_scheduler()
        try:
            scheduler.remove_job("tools_health_check")
        except:
            pass
        
        scheduler.add_job(
            tools_health_check_job,
            'interval',
            minutes=10,
            id="tools_health_check",
            name="Tools Health Check - verifica tools a cada 10 min",
            replace_existing=True
        )
        print(f"[Scheduler] Tools Health job agendado a cada 10 min")
        return True
    except Exception as e:
        print(f"[Scheduler] Erro ao iniciar tools health scheduler: {e}")
        return False

def start_all_health_schedulers():
    """Inicia todos os schedulers de health"""
    start_provider_health_scheduler()
    start_tools_health_scheduler()
